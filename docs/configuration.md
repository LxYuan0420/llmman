...
| `LLMMAN_CONTEXT_LENGTH` | Context size for llama-server (`--ctx-size`) and vLLM (`--max-model-len`) model loads. Defaults to a VRAM-tiered value when unset; `mlx_lm.server` has no equivalent setting. |
