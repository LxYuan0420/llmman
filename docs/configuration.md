...
| `LLMMAN_CONTEXT_LENGTH` | Positive context size for model loads. When set, llmman passes it to llama-server as `--ctx-size` and to vLLM as `--max-model-len`. When unset or invalid, llama-server may use llmman’s VRAM-tiered default, while vLLM keeps its model-derived default. `mlx_lm.server` has no equivalent setting. |
